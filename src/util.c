...
char *concat_with_colon(char *front, char *back) {
  char *with_semicolon = concat(":", back);
  char *full_name = concat(front, with_semicolon);
  return full_name;
}

/** appendstr (Append String). Appends the string 'append' to the string
 * 'string'.
...
/**
 * strtokk (String Tokenizer). Breaks string 'string' into a series of tokens
 * using 'strf' as a WHOLE DELIMITER string. It works in the same way as strtok,
 * so the 'string' passed as parameter is split in the different parts, breaking
 * the 'string'.
 * @param string The contents of this string are modified and broken into
 * smaller strings (tokens).
 * @param strf This is the C string containing the DELIMITER.
 * @return Returns a pointer to the first token found in the string. A null
...
/**
 * strrep (String Replace). Replaces 'strf' with 'strr' in 'cadena' and returns
 * the new string. You need to free the returned string in your code after using
 * strrep.
